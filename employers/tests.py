...
class EmployerTestCase(TestCase):
    ...
    def test_sign_up(self):
        ...
        response = self.client.post(reverse('employers:register'),
                                    {'first_name': first_name, 'last_name': last_name, 'email': email,
                                     'username': username, 'password': password,
                                     'password_confirmation': password_confirmation, 'company_name': company_name,
                                     'company_website': company_website, 'phone_number': phone_number})

        self.assertRedirects(response, '/employers/email_confirmation/' + email, status_code=302,
